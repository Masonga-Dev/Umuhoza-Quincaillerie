...
function LanguageSwitcher() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-amber-400"
      >
          ...
          <div className="absolute right-0 z-50 mt-2 w-40 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
            {LANGS.map((l) => (
              <button key={l.code} onClick={() => { setLang(l.code); setOpen(false); }}
                className={`flex w-full items-center gap-3 px-4 py-2.5 text-sm transition hover:bg-slate-50 ${lang === l.code ? 'bg-blue-50 font-semibold text-blue-700' : 'text-slate-700'}`}>
            ...
            {menuOpen && (
              <div className="lg:hidden border-t border-slate-100 py-3">
                ...
                <div className="mt-3 border-t border-slate-100 pt-3 flex flex-col gap-2">
                  <NavLink to="/contact" className="block rounded-xl bg-[#1a2d5a] px-4 py-2.5 text-center text-sm font-semibold text-white transition hover:bg-amber-500">
                    Request a Quote
                  </NavLink>
        ...
        <>
          ...
          <footer className="relative overflow-hidden bg-slate-950 text-slate-200">
            ...
            <div style={{ position:'relative', zIndex:1 }} className="mx-auto grid max-w-7xl gap-4 px-4 py-5 sm:px-6 sm:grid-cols-2 lg:grid-cols-[1.3fr_1fr_1fr_1.2fr] lg:gap-6 lg:py-6">
              ...
              {/* Categories */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-amber-400">{t('footer.categories')}</p>
                <ul className="mt-2.5 space-y-1.5 text-xs text-slate-400">
                  {categories.slice(0, 5).map((cat) => (
                    <li key={cat.id}>
                      <NavLink to={`/products?category=${cat.id}`} className="transition hover:text-amber-400 line-clamp-1">{cat.name}</NavLink>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {/* Contact Info */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-amber-400">{t('footer.contactInfo')}</p>
                <div className="mt-2.5 space-y-2 text-xs text-slate-400">
                  <a href="https://maps.app.goo.gl/E6aSqZ6aDYa39hH1A" target="_blank" rel="noreferrer" className="flex items-start gap-1.5 transition hover:text-amber-400">
                  ...
                  <a href={`tel:${phone}`} className="flex items-center gap-1.5 transition hover:text-amber-400">
                    <svg className="h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    <span>{phone}</span>
                  </a>
                  <a href={`mailto:${email}`} className="flex items-center gap-1.5 transition hover:text-amber-400">
                    <svg className="h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                    <span className="break-all">{email}</span>
                  </a>
                  <div className="flex items-center gap-1.5">
                    <svg className="h-3.5 w-3.5 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
