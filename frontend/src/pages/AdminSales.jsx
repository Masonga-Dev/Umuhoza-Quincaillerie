...
export default function AdminSales() {
  ...
  const paginatedSales = filtered.slice((page - 1) * pageSize, page * pageSize);
  ...
  const handleSuccess = invoice => {
    setShowForm(false);
    setSuccess(`${t('admin.salesPage.saleRecorded')} ${invoice}`);
    loadSales();
    emitDataChanged();
    ...
    <AdminLayout currentPage="/admin/sales">
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{t('admin.salesPage.title')}</h2>
            <p className="mt-1 text-sm text-slate-500">{t('admin.salesPage.subtitle')}</p>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(true)}
              ...
              <span className="text-lg leading-none">+</span> {t('admin.salesPage.recordNewSale')}
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-slate-500">{t('admin.salesPage.todayRevenue')}</p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-slate-500">{t('admin.salesPage.totalRevenue')}</p>
            ...
            <p className="mt-1 text-xs text-slate-400">{t('admin.salesPage.allTimeExcludingCancelled')}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-slate-500">{t('admin.salesPage.cashRevenue')}</p>
            ...
            <p className="mt-1 text-xs text-emerald-500 font-semibold">{t('admin.salesPage.cash')}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <p className="text-sm font-medium text-slate-500">{t('admin.salesPage.mobileMoneyRevenue')}</p>
            ...
            <p className="mt-1 text-xs text-purple-500 font-semibold">{t('admin.salesPage.mobileMoney')}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
          <div className="relative flex-1 min-w-0 sm:min-w-[180px]">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('admin.salesPage.searchPlaceholder')}
          ...
          <select value={paymentFilter} onChange={e => setPaymentFilter(e.target.value)}
            ...
            <option value="">{t('admin.salesPage.allMethods')}</option>
            <option value="Cash">Cash</option>
            <option value="Mobile Money">Mobile Money</option>
            <option value="Bank Transfer">Bank Transfer</option>
          </select>
          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-slate-500 whitespace-nowrap">{t('admin.salesPage.from')}</label>
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
              className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-blue-400" />
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-slate-500 whitespace-nowrap">{t('admin.salesPage.to')}</label>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
            <h3 className="font-semibold text-slate-900">{t('admin.salesPage.history')}</h3>
            {!loading && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{filtered.length} {t('admin.salesPage.records')}</span>}
          </div>
          ...
          {loading ? (
            ...
            <div className="flex h-48 flex-col items-center justify-center gap-3 text-slate-400">
              <p className="text-sm font-semibold">{t('admin.salesPage.noSalesRecords')}</p>
            </div>
          ) : (
