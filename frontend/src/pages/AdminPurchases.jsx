  ...
  const handleExport = async (period) => {
    try {
      ...
      if (typeof period === 'object' && period.period === 'custom') {
        params = { period: 'custom', from: period.from.toISOString().slice(0, 10), to: period.to.toISOString().slice(0, 10) };
      } else if (period && period !== 'all') {
        params = { period };
      }
      const { data = [] } = await API.get('/purchases/export', { headers: HEADERS(), params });
                  ...
                  {filtered.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-5 py-4 text-right font-bold text-slate-900">
                        {fmt(Number(p.total_cost) - Number(p.total_returned_cost || 0))} <span className="text-xs font-normal text-slate-400">RWF</span>
                      </td>
